...
public class CSVParser {
    ...
    private boolean useWhitespacesAsSeparator = false;
    ...
    public CSVParser(char separator, char quotechar, char escape, boolean strictQuotes, boolean ignoreLeadingWhiteSpace) {
        if (separator == NULL_CHARACTER) {
            useWhitespacesAsSeparator = true;
    ...
    private String[] parseLine(String nextLine, boolean multi) throws IOException {
        ...
        for (int i = 0; i < nextLine.length(); i++) {
            ...
            if (c == this.escape) {
                ...
                if (isNextCharacterEscapedQuote(nextLine, inQuotes || inField, i)) {
                    ...
                    if (!strictQuotes) {
                        if (i > 2 //not on the beginning of the line
                                && (nextLine.charAt(i - 1) != this.separator && !useWhitespacesAsSeparator) //not at the beginning of an escape sequence
                                && nextLine.length() > (i + 1)
                                && (nextLine.charAt(i + 1) != this.separator && !useWhitespacesAsSeparator)//not at the	end of an escape sequence
                                ) {
                ...
                inField = !inField;
            } else if (!useWhitespacesAsSeparator && c == separator && !inQuotes) {
                tokensOnThisLine.add(sb.toString());
                sb.setLength(0); // start work on next token
                inField = false;
            } else if (useWhitespacesAsSeparator && Character.isWhitespace(c) && !inQuotes) { // for R compatibility
                if (sb.length() > 0) {
                    tokensOnThisLine.add(sb.toString());
                    sb.setLength(0); // start work on next token
                }
                ...
                for (int j = i + 1; j < nextLine.length(); j++) {
                    if (Character.isWhitespace(nextLine.charAt(j))) {
                        i = j;
                    }else{
                        break;
                    }
